...
def reward_function_waypoints(params):
    ...
    progress_reward = params['progress'] / params['steps'] if params['steps'] > 0 and direction_diff <= 20 else 1e-6
    if params['progress'] == 100:
        reward += 100 * params['progress'] / params['steps']
    reward = ( reward + progress_reward )/ 2
    trace_log = f'"reward":{reward},"curve_angle":{curve_angle},"speed_penalty":{speed_penalty},"safe_max_speed":{safe_max_speed},"direction_diff":{direction_diff},"progress_reward":{progress_reward}'
