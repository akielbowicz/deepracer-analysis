...
class Reward(RewardBase):
    ...
    def motion_reward(self):
        return exp( self.base_reward()
                + self.penalize_steps() 
                + self.follow_the_center_line() ) / (self.steps + 1)
                
    def follow_the_center_line(self):
        return exp(-2*self.distance_from_center**2/self.track_width**2)
        
    def is_on_track(self):
        return self.motion_reward() + self.progress_reward() + self.advance_reward() + self.control_steering() if self.all_wheels_on_track else -0.1
